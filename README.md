...
- Local training with Adam, stable sparse cross-entropy, pre-apply NaN/Infinity guards, gradient clipping, AdamW-style decay, random batched context windows, and TensorFlow.js WebGL acceleration when available.
- Chat UI with top-k/nucleus sampling, temperature, repetition penalty, loop blocking, faster batched UI streaming, and stop-generation support.
- Compact base64 JSON import/export for trained weights, plus NaN/Infinity repair so corrupted weights do not export as `null`.
